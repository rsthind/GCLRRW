...
class LightGCL(nn.Module):
    def __init__(self, n_u, n_i, d, train_csr, adj_norm, l, temp, lambda_1, lambda_2, dropout, batch_user, device, ret_adj_norm):
        ...
        self.ret_adj_norm = ret_adj_norm
    ...
    def forward(self, uids, iids, pos, neg, test=False):
        if test==True:  # testing phase
            ...
            for layer in range(1,self.l+1):
                # GNN propagation
                self.E_u_list[layer] = (torch.spmm(sparse_dropout(self.adj_norm,self.dropout), self.E_i_list[layer-1]))
                self.E_i_list[layer] = (torch.spmm(sparse_dropout(self.adj_norm,self.dropout).transpose(0,1), self.E_u_list[layer-1]))
